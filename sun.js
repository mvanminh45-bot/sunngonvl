...
app.get('/sun', async (req, res) => {
  try {
    ...
    const last50Results = sunData.slice(0, 50).map(d => d.Ket_qua === 'Tài' ? 'T' : 'X');
    const cau_pattern = last50Results.join('');
    
    const patternDetails = result.allPatterns.map(p => ({
      name: p.name || 'Unknown',
    ...
    res.json({
      ...
      id: '@vanminh2603',
      cau_pattern: cau_pattern,
      reversed: result.reversed || false,
      factors: result.factors || [],
      pattern_details: patternDetails,
      detailed_analysis: {
        total_patterns_detected: result.allPatterns ? result.allPatterns.length : 0,
        top_pattern: result.detailedAnalysis?.topPattern || 'N/A',
        tai_score: result.detailedAnalysis?.taiScore || 0,
        xiu_score: result.detailedAnalysis?.xiuScore || 0,
        adaptive_boost: result.detailedAnalysis?.adaptiveBoost || 0,
        current_streak: result.detailedAnalysis?.learningStats?.currentStreak || 0,
        accuracy: result.detailedAnalysis?.learningStats?.accuracy || 'N/A'
      }
    });
  } catch (error) {
...
app.get('/sun/lichsu', async (req, res) => {
  ...
  try {
    ...
    const historyWithStatus = predictionHistory.b52.map(record => {
      const prediction = learningData.b52.predictions.find(p => p.phien === record.phien_hien_tai);
      let status = null;
    ...
    if (format === 'html') {
      ...
      const streak = learningData.b52.streakAnalysis;
      const reversal = learningData.b52.reversalState || { active: false, totalReversals: 0 };

      const rows = historyWithStatus.slice(0, 20).map(r => `
        <tr>
          <td style="padding: 8px; border: 1px solid #2a2a5a;">${r.phien}</td>
          <td style="padding: 8px; border: 1px solid #2a2a5a; font-weight: bold; color: ${r.du_doan === 'tai' ? '#66ff88' : '#ff6666'};">${r.du_doan.toUpperCase()}</td>
          <td style="padding: 8px; border: 1px solid #2a2a5a;">${r.ket_qua_thuc_te || '---'}</td>
          <td style="padding: 8px; border: 1px solid #2a2a5a; text-align: center;">${r.status || '<span class="status-pending">⏳ Chờ</span>'}</td>
          <td style="padding: 8px; border: 1px solid #2a2a5a; text-align: center;">${r.ti_le}</td>
        </tr>
      `).join('');
      ...
      const html = `
        ...
        <head>
          ...
          <title>📊 Sun Tài Xỉu - Dashboard</title>
          <style>
            * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Courier New', monospace; }
            body { background: #0a0a1a; color: #e0e0e0; padding: 20px; display: flex; justify-content: center; }
            .container { max-width: 1000px; width: 100%; background: #12122a; border-radius: 16px; padding: 24px; border: 1px solid #2a2a5a; box-shadow: 0 0 40px rgba(0,100,255,0.1); }
            ...
            .badge-on { background: #1a3a1a; color: #66ff88; border: 1px solid #66ff88; }
            .badge-off { background: #3a1a1a; color: #ff6666; border: 1px solid #ff6666; }
          </style>
        </head>
        <body>
          <div class="container">
            ...
            <div class="stats-grid">
              ...
              <div class="stat-box"><div class="num ${reversal.active ? 'gold' : 'gray'}">${reversal.active ? '✅ BẬT' : '❌ TẮT'}</div><div class="label">🔄 Auto-Reversal (${reversal.totalReversals || 0})</div></div>
            </div>
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('  GET /sun - Get prediction for next Sun round (with cau_pattern)');
  console.log('  GET /sun/lichsu - Get prediction history (JSON)');
  console.log('  GET /sun/lichsu?format=html - Get dashboard (HTML)');
